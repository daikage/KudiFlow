...
@section('content')
  <div class="min-h-[80vh] flex items-center justify-center">
    <div class="w-full max-w-2xl bg-surface-container-lowest rounded-2xl p-8 shadow border border-outline-variant/10">
      ...
      <p class="text-on-surface-variant mb-6">Start a 14-day trial (all features) or select a plan to activate immediately.</p>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="rounded-xl border border-outline-variant/10 p-5 bg-surface-container">
          ...
          <p class="text-sm text-on-surface-variant mt-1">Try all features free for 14 days.</p>
        ...
        <div class="rounded-xl border border-outline-variant/10 p-5 bg-surface-container">
          ...
          <form method="POST" action="{{ route('subscriptions.choose_plan') }}" class="mt-4 space-y-3">
            @csrf
            @if($plans->isEmpty())
              <div class="text-sm text-on-surface-variant">No plans available yet. Please start a trial.</div>
            @else
              <select name="plan" class="w-full bg-surface-container-low border border-outline-variant/30 rounded-lg px-3 py-2">
                @foreach($plans as $p)
                  <option value="{{ $p->code }}">
                    {{ $p->name }} — ₦{{ number_format($p->price, 2) }} / {{ $p->interval }}
                  </option>
                @endforeach
              </select>
      ...
      <p class="text-xs text-on-surface-variant mt-6">This step is required only once per company. Super Admins are not affected.</p>
    </div>
  </div>
@endsection
