...
@section('content')
  <div class="bg-surface-container-lowest rounded-xl p-6 shadow-sm border border-outline-variant/5">
    ...
    <form method="POST" action="{{ route('sa.subscriptions.update', $plan) }}" class="grid grid-cols-1 md:grid-cols-2 gap-4">
      @csrf @method('PUT')

      <div>
        <label class="block text-[12px] font-bold text-on-surface-variant uppercase tracking-widest mb-1">Code</label>
        <input class="w-full bg-surface-container-low border border-outline-variant/30 rounded-lg px-3 py-2" value="{{ $plan->code }}" disabled>
      </div>
      <div>
        <label class="block text-[12px] font-bold text-on-surface-variant uppercase tracking-widest mb-1">Name</label>
        <input name="name" class="w-full bg-surface-container-low border border-outline-variant/30 rounded-lg px-3 py-2" value="{{ old('name', $plan->name) }}" required>
      </div>
      <div>
        <label class="block text-[12px] font-bold text-on-surface-variant uppercase tracking-widest mb-1">Price</label>
        <input name="price" inputmode="decimal" class="w-full bg-surface-container-low border border-outline-variant/30 rounded-lg px-3 py-2" value="{{ old('price', $plan->price) }}" required>
      </div>
      <div>
        ...
        <select name="interval" class="w-full bg-surface-container-low border border-outline-variant/30 rounded-lg px-3 py-2">
          <option value="monthly" @selected(old('interval',$plan->interval)==='monthly')>Monthly</option>
      ...
      <div>
        ...
        <input name="sort" inputmode="numeric" class="w-full bg-surface-container-low border border-outline-variant/30 rounded-lg px-3 py-2" value="{{ old('sort', $plan->sort) }}">
      </div>

      <div class="md:col-span-2">
        @php $ent = $plan->entitlements ?? []; @endphp
        <p class="text-[12px] font-bold text-on-surface-variant uppercase tracking-widest mb-2">Entitlements</p>
        <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
          @foreach($modules as $m)
            <label class="inline-flex items-center gap-2">
              <input type="checkbox" name="entitlements[{{ $m }}]" value="1" @checked(old('entitlements.'.$m, $ent[$m] ?? false))>
      ...
      <div class="md:col-span-2 flex gap-2">
        ...
        <button type="submit" class="px-4 py-2 rounded-lg bg-primary text-on-primary shadow-sm">Update Plan</button>
      </div>
    </form>
  </div>
@endsection
