    ...
    <div class="bg-surface-container-lowest rounded-xl p-6 shadow-sm border border-outline-variant/5">
      ...
      <form action="{{ route('ui.products.update', $product) }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input name="barcode" value="{{ old('barcode', $product->barcode) }}" class="w-full bg-surface-container-low border @error('barcode') border-error @else border-outline-variant/30 @enderror rounded-lg px-3 py-2" placeholder="EAN-13/Code-128">
